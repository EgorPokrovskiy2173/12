...
Процедура ПриОткрытии(Отказ)
	...
	Если ОшибкаПодключения Тогда
		...
		ОткрытьФорму("ОбщаяФорма.БЗ_ФормаНастроекПроксиСервера", ПараметрыФормы,,,,,ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ПроверитьТекстЧерезСервис()
	...
	Попытка
		ВебСервис = WSСсылки.БЗ_YandexSpeller.СоздатьWSПрокси("http://speller.yandex.net/services/spellservice", "SpellService", "SpellServiceSoap12");
	Исключение
