...
Процедура ДополнитьПолучателейНовостиПоВидуИОбъекту(Получатели, ВидНовости, ОбъектБазыЗнаний)
	...
	Если ТипЗнч(ОбъектБазыЗнаний) = Тип("СправочникСсылка.БЗ_СтатьиБазыЗнаний") Тогда
		МассивКатегорий = ПолучитьМассивКатегорийПоСтатье(ОбъектБазыЗнаний);
	ИначеЕсли ТипЗнч(ОбъектБазыЗнаний) = Тип("СправочникСсылка.БЗ_КомментарииБазыЗнаний") Тогда
	...
	ИначеЕсли ТипЗнч(ОбъектБазыЗнаний) = Тип("СправочникСсылка.БЗ_КатегорииБазыЗнаний") Тогда
		МассивКатегорий = Новый Массив;
		...
		Если МассивКатегорий.Количество() = 1 И ВидНовости = Перечисления.БЗ_ВидыНовостейБазыЗнаний.Создание Тогда
	...
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	БЗ_ИзбранноеБазыЗнаний.Пользователь
	|ИЗ
	|	РегистрСведений.БЗ_ИзбранноеБазыЗнаний КАК БЗ_ИзбранноеБазыЗнаний
	|ГДЕ
	|	БЗ_ИзбранноеБазыЗнаний.КатегорияБазыЗнаний В(&Категории)
	|	И БЗ_ИзбранноеБазыЗнаний.Пользователь <> &ТекущийПользователь
	|	И ИСТИНА";
...
Функция ПолучитьОбъектНовостьБазыЗнаний(Автор, Заголовок, ОбъектБазыЗнаний)
	...
	|ГДЕ
	|	БЗ_НовостиБазыЗнаний.Наименование = &Заголовок
	|	И БЗ_НовостиБазыЗнаний.Дата МЕЖДУ &НачалоДня И &КонецДня
	|	И БЗ_НовостиБазыЗнаний.ОбъектБазыЗнаний = &ОбъектБазыЗнаний
	|	И БЗ_НовостиБазыЗнаний.Автор = &Автор";
	Результат = Запрос.Выполнить();
	
	Если Результат.Пустой() Тогда
	    Возврат Справочники.БЗ_НовостиБазыЗнаний.СоздатьЭлемент();
	Иначе 
		Выборка = Результат.Выбрать();
		Выборка.Следующий();
...
Функция ДобавитьНовость(знач Автор, знач Заголовок, знач ТекстHTML, знач ОбъектБазыЗнаний = Неопределено, знач ВидНовости = Неопределено, Получатели = Неопределено, знач Важность = 2, знач Текст = "") Экспорт 
	...
	Если ВидНовости = Неопределено Тогда
		ВидНовости = Перечисления.БЗ_ВидыНовостейБазыЗнаний.Прочее; 
	КонецЕсли;
	...
	СоздаватьОбщуюЗапись = (ВидНовости = Перечисления.БЗ_ВидыНовостейБазыЗнаний.Оповещение
		ИЛИ ВидНовости = Перечисления.БЗ_ВидыНовостейБазыЗнаний.Прочее);
	...
	Если Получатели.Количество() = 0 Тогда
		Возврат Справочники.БЗ_НовостиБазыЗнаний.ПустаяСсылка();
	КонецЕсли;
