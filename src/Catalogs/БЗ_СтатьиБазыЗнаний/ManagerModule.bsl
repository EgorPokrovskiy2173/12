...
Функция ЗахватитьСтатьюНаРедактирование(знач СтатьяСсылка, знач Область = "", ТекПользователь = Неопределено) Экспорт
	...
	ТекущийРедактор = БЗ_БазаЗнаний.ПолучитьДанныеБлокировки(СтатьяСсылка, Область).Пользователь;
	...
	ЭлементБлокировки = БлокировкаДанных.Добавить("РегистрСведений.БЗ_БлокировкаСтатьейБазыЗнаний");
	...
	Попытка
		...
		МенеджерЗаписи = РегистрыСведений.БЗ_БлокировкаСтатьейБазыЗнаний.СоздатьМенеджерЗаписи();
...
Функция ЗавершитьРедактированиеСтатьи(знач СтатьяСсылка, знач Область = "") Экспорт
	...
	ТекущийРедактор = БЗ_БазаЗнаний.ПолучитьДанныеБлокировки(СтатьяСсылка, Область).Пользователь;
	Администратор	= БЗ_БазаЗнанийКлиентСерверПовтИсп.ПолучитьЗначениеОбщейНастройки("Администратор");
	ТекПользователь	= Пользователи.ТекущийПользователь();
...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Если ВидФормы = "ФормаОбъекта" Тогда
		Если Параметры.Свойство("Ключ") Тогда
			...
			ВыбраннаяФорма = "Обработка.БЗ_БазаЗнаний.Форма";
