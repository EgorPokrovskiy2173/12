...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(ЭтотОбъект.Область) Тогда
		СтатьяЗахвачена = Справочники.БЗ_СтатьиБазыЗнаний.ЗахватитьСтатьюНаРедактирование(ЭтотОбъект.СтатьяБазыЗнаний);
	...
	// Потому что в ЗаполнитьВложенныеСтрокиДереваКатегорий() будет по этим категориям поиск для флага КатегорииДерево.Использование
	Если Параметры.Свойство("ЗначенияЗаполнения") Тогда
		ЗначенияЗаполнения = Параметры.ЗначенияЗаполнения;
		Если ЗначенияЗаполнения.Свойство("Категория") Тогда
			Категория = ЗначенияЗаполнения.Категория;
			Если ЗначениеЗаполнено(Категория) Тогда
				ПараметрыОтбора = Новый Структура("Категория", Категория);
				НайденныеСтроки = Категории.НайтиСтроки(ПараметрыОтбора);
				
				Если НайденныеСтроки.Количество() = 0 Тогда
					НоваяКатегория = Категории.Добавить();
					НоваяКатегория.Категория = Категория;
				КонецЕсли;	
			КонецЕсли;
		КонецЕсли;		
	КонецЕсли; 
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Привязки, "Статья", СтатьяБазыЗнаний, ВидСравненияКомпоновкиДанных.Равно,,Истина,РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный);
	//--- АйТи Кучеров Р.М. 07.10.2020 ТЗ № ИС00-003398 >> e1cib/data/Документ.ЗаданиеНаРаботу?ref=82a932336337376211eb089480e29675
	//--- АйТи КучеровРМ 06.11.2018 ТЗ № ИС00-002921 
	...
	Если ЗначениеЗаполнено(СтатьяБазыЗнаний) Тогда
		ЗагрузитьСписокПользователейДляОповещания();
	КонецЕсли;
	
	//Если СтатьяБазыЗнаний.Пустая() Тогда
	//	ВыдатьНаИзучениеЗаново = Ложь;
	//	Элементы.СписокПользователейДляОповещания.Видимость = Ложь;
	//Иначе
	//	ВыдатьНаИзучениеЗаново = Ложь;
	//	Элементы.СписокПользователейДляОповещания.Видимость = Истина;
	//КонецЕсли;
	//+++ АйТи Горячёв В.С. 15.04.21
	
	//+++ АйТи Суворова С. О. 06.02.2023 ТЗ № НФ00-003414 >> e1cib/data/Документ.ЗаданиеНаРаботу?ref=a935fc3497141b3f11ed82a9ee59bafc
	Если ЗначениеЗаполнено(СтатьяБазыЗнаний.СтатусСтатьи) Тогда
		СтатусСтатьи = СтатьяБазыЗнаний.СтатусСтатьи;
		Элементы.СтатусСтатьи.СписокВыбора.Добавить(Перечисления.БЗ_СтатусыСтатьи.НеЗагружать);
		Элементы.СтатусСтатьи.СписокВыбора.Добавить(Перечисления.БЗ_СтатусыСтатьи.Загружать);
		ТекПользователь = Пользователи.ТекущийПользователь();
		ТекСотрудник = БЗ_ОбщиеФункцииСерверПовтИсп.ОпределитьСотрудникаПоПользователю(ТекПользователь);
	КонецЕсли;
	//Если ТекСотрудник = Константы.АйТи_Маркетолог.Получить() Тогда
	//	Элементы.СтатусСтатьи.СписокВыбора.Добавить(Перечисления.АйТи_СтатусыСтатьи.УКопирайтера);
	//	Элементы.СтатусСтатьи.СписокВыбора.Добавить(Перечисления.АйТи_СтатусыСтатьи.Загружена);
	//КонецЕсли;
	//Информер = АйТи_ПрограммноеФормированиеИнтерфейса.СформироватьВсплывающийИнформерСтатусаСтатьи(ЭтаФорма, Элементы.Найти("ГруппаСтатус"), "ИнформерСтатусСтатьи");
	//--- АйТи Суворова С. О. 06.02.2023 ТЗ № НФ00-003414 >> e1cib/data/Документ.ЗаданиеНаРаботу?ref=a935fc3497141b3f11ed82a9ee59bafc   
	
	ВыдатьНаИзучениеЗаново = Истина;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	//+++ АйТи Тимошечкин О.А. 15.02.2022 ТЗ № ИС00-000692 >> e1cib/data/Документ.ЗаданиеНаРаботу?ref=a914fc3497141b3f11ec8326e4ea2539
	Если (ИмяСобытия = "ПроверкаПравописания_ИзмененТекст") И (Источник = ЭтаФорма) Тогда
		Если Параметр.ИмяРеквизита = "ТекстСтатьи" Тогда
			ОбновитьТекстСтатьиHTML(Параметр.Текст, Параметр.Вложения);
		КонецЕсли;
		Модифицированность = Истина;
	КонецЕсли;
	//--- АйТи Тимошечкин О.А. 15.02.2022 ТЗ № ИС00-000692 >> e1cib/data/Документ.ЗаданиеНаРаботу?ref=a914fc3497141b3f11ec8326e4ea2539
...
Процедура ВставитьКартинку(Команда)
	...
	Если МассивЭлементов.Количество() > 0 И ТипЗнч(МассивЭлементов[0]) = Тип("ТекстФорматированногоДокумента") Тогда
		...
		Если ЗакладкаНачало = ЗакладкаКонец И БЗ_БазаЗнанийКлиентСервер.ЭтоСтрокаКартинки(ЭлементФорматированногоДокумента.Текст) Тогда
	...
	ИмяКартинки		= БЗ_БазаЗнанийКлиентСервер.ПолучитьИмяКартинки(ВыделенныйТекст);
...
Процедура ВставитьТаблицу(Команда)
	...
	Если МассивЭлементов.Количество() > 0 И ТипЗнч(МассивЭлементов[0]) = Тип("ТекстФорматированногоДокумента") Тогда
		ЭлементФорматированногоДокумента = МассивЭлементов[0];
		Если ЗакладкаНачало = ЗакладкаКонец И БЗ_БазаЗнанийКлиентСервер.ЭтоСтрокаТаблицы(ЭлементФорматированногоДокумента.Текст) Тогда
			ЗакладкаНачало	= ЭлементФорматированногоДокумента.ЗакладкаНачала;
			ЗакладкаКонец	= ЭлементФорматированногоДокумента.ЗакладкаКонца;
			ВыделенныйТекст	= ЭлементФорматированногоДокумента.Текст;
		КонецЕсли;
	...
	ИмяТаблицы		= БЗ_БазаЗнанийКлиентСервер.ПолучитьИмяТаблицы(ВыделенныйТекст);
...
Процедура ИсторияИзменений(Команда)
	...
	ОткрытьФорму("Справочник.БЗ_СтатьиБазыЗнаний.Форма.ПросмотрИстории", ПараметрыФормы, ЭтаФорма);
...
Процедура ЗаписатьИЗакрыть(Команда)
	...
	//+++ АйТи КучеровРМ 02.12.2018 ТЗ № ИС00-002921
	// Причина заменты кода: чтобы не мучать пользователя надоедливыми диалогами 
	//+++ Было до 02.12.2018 22:29:24 // Далее тут старый код, который теперь заменён
	...
	//КонецЕсли;
	//--- Было до 02.12.2018 22:29:24 
	ОсвободитьСтатью = Истина; // [ АйТи КучеровРМ ] Справа было до 02.12.2018 23:00:46: НЕ ЭтоПодраздел И (Результат = КодВозвратаДиалога.Да  
	
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("ОсвободитьСтатью", ОсвободитьСтатью);
	
	СимволовСтатьи = СтрДлина(ТекстСтатьи.ПолучитьТекст());
	ЗаписатьИЗакрытьЗавершение("Символов в статье: " + СимволовСтатьи, ПараметрыОповещения);
	//--- АйТи КучеровРМ 02.12.2018 ТЗ № ИС00-002921
...
Процедура ПредварительныйПросмотр(Команда)
	...
	Записать(Неопределено);
	Если ЗначениеЗаполнено(СтатьяБазыЗнаний) Тогда
		ОткрытьЗначение(СтатьяБазыЗнаний);
	КонецЕсли;
...
Процедура ПостроитьДеревоКатегорииНаСервере()
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	БЗ_КатегорииБазыЗнаний.Родитель КАК Родитель,
	|	БЗ_КатегорииБазыЗнаний.Ссылка,
	|	БЗ_КатегорииБазыЗнаний.РеквизитДопУпорядочивания
	|ИЗ
	...
	РодительСсылка = Справочники.БЗ_КатегорииБазыЗнаний.ПустаяСсылка();
...
Процедура ВыполнитьОформлениеФорматированногоДокумента()
	...
	Возврат;
	//+++ АйТи КучеровРМ 03.08.2018 ТЗ № ИС00-002921
	//Причина заменты кода: внутри их вечный цикл. И т.к. при оформлении вылетает, то вообще без него 
	//+++Было до 03.08.2018 17:41:18 // Далее тут старый код, который теперь заменён
...
Процедура ЗаполнитьДанныеФормыПоСсылке()
	...
	Иначе 
		...
		|	БЗ_СтатьиБазыЗнаний.ПоложениеСодержания КАК ПоложениеСодержания,
	...
	"ВЫБРАТЬ
	|	БЗ_ХранилищеДанныхБазыЗнаний.СтатьяБазыЗнаний КАК Ссылка,
	|	БЗ_ХранилищеДанныхБазыЗнаний.Область,
	|	БЗ_ХранилищеДанныхБазыЗнаний.Идентификатор КАК ИдентификаторКартинки,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.БЗ_СтатьиБазыЗнаний.Картинки КАК СтатьиБазыЗнанийКартинки
	|		ПО БЗ_ХранилищеДанныхБазыЗнаний.СтатьяБазыЗнаний = СтатьиБазыЗнанийКартинки.Ссылка
	|			И БЗ_ХранилищеДанныхБазыЗнаний.Область = СтатьиБазыЗнанийКартинки.Область
	|			И БЗ_ХранилищеДанныхБазыЗнаний.Идентификатор = СтатьиБазыЗнанийКартинки.Идентификатор
	|ГДЕ
	...
	Если ЗначениеЗаполнено(ЭтотОбъект.Область) Тогда
		ЗапросКартинки.Текст = СтрЗаменить(ЗапросКартинки.Текст, "1 = 1", "БЗ_ХранилищеДанныхБазыЗнаний.Область = &Область");
	КонецЕсли;
	...
	|	БЗ_ХранилищеДанныхБазыЗнаний.СтатьяБазыЗнаний КАК Ссылка,
	|	БЗ_ХранилищеДанныхБазыЗнаний.Область,
	|	БЗ_ХранилищеДанныхБазыЗнаний.Идентификатор КАК ИдентификаторТаблицы,
	...
	|ИЗ
	|	РегистрСведений.БЗ_ХранилищеДанныхБазыЗнаний КАК БЗ_ХранилищеДанныхБазыЗнаний
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.БЗ_СтатьиБазыЗнаний.Таблицы КАК СтатьиБазыЗнанийТаблицы
	|		ПО БЗ_ХранилищеДанныхБазыЗнаний.СтатьяБазыЗнаний = СтатьиБазыЗнанийТаблицы.Ссылка
	|			И БЗ_ХранилищеДанныхБазыЗнаний.Область = СтатьиБазыЗнанийТаблицы.Область
	|			И БЗ_ХранилищеДанныхБазыЗнаний.Идентификатор = СтатьиБазыЗнанийТаблицы.Идентификатор
	|ГДЕ
	|	БЗ_ХранилищеДанныхБазыЗнаний.СтатьяБазыЗнаний = &Ссылка
	|	И БЗ_ХранилищеДанныхБазыЗнаний.ВидДанных = 2
	|	И 1 = 1";
	
	Если ЗначениеЗаполнено(ЭтотОбъект.Область) Тогда
		ЗапросТаблицы.Текст = СтрЗаменить(ЗапросТаблицы.Текст, "1 = 1", "БЗ_ХранилищеДанныхБазыЗнаний.Область = &Область");
	КонецЕсли;
	...
	|ИЗ
	|	Справочник.БЗ_СтатьиБазыЗнаний.Содержание КАК СтатьиБазыЗнанийСодержание
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.БЗ_СтатьиБазыЗнаний.Содержание КАК СтатьиРодители
...
Процедура ИнициализацияДанных(ПриСоздании = Истина)
	...
	Если ПриСоздании И Параметры.ЗаполнитьИзПараметров Тогда
		ЗаполнитьДанныеФормыПоПараметрам();
	ИначеЕсли ЗначениеЗаполнено(СтатьяБазыЗнаний) Тогда
		ЗаполнитьДанныеФормыПоСсылке();
	КонецЕсли;
	
	СтруктураКартинок = ПолучитьСтруктуруКартинок();
	ЭтотОбъект.ТекстСтатьи.УстановитьHTML(ЭтотОбъект.ТекстHTML, СтруктураКартинок);
...
Процедура ЗаписатьИЗакрытьЗавершение(Результат, ДопПараметры) Экспорт
	...
	//+++ АйТи Кучеров Р.М. 13.10.2020 ТЗ № ИС00-001146 >> e1cib/data/Документ.ЗаданиеНаРаботу?ref=a17f32336337376211e9249202ccc0a0
	лНовый = СтатьяБазыЗнаний.Пустая();
	//--- АйТи Кучеров Р.М. 13.10.2020 ТЗ № ИС00-001146 >> e1cib/data/Документ.ЗаданиеНаРаботу?ref=a17f32336337376211e9249202ccc0a0
	
	ПередЗаписью(Истина, ДопПараметры);
КонецПроцедуры
...
Процедура ЗаписатьДанныеКлючевыхСлов()
	...
	НаборЗаписей = РегистрыСведений.БЗ_СвязьКлючевыхСловБазыЗнаний.СоздатьНаборЗаписей();
	...
	Для Каждого СтрокаТаблицы Из ЭтотОбъект.КлючевыеСлова Цикл
		...
		Если НЕ ЗначениеЗаполнено(КлючевоеСловоСсылка) Тогда
			КлючевоеСловоСсылка = Справочники.БЗ_КлючевыеСловаБазыЗнаний.ПолучитьКлючевоеСловоПоНаименованию(СтрокаТаблицы.Наименование);
		КонецЕсли;
	...
	Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
		...
		"БЗ_БазаЗнаний.ОбновитьРейтингКлючевыхСлов",
		ПараметрыВыполнения,
		"Обновление рейтинга ключевых слов");
...
Процедура ЗавершитьРедактированиеСтатьиНаСервере(СтатьяБазыЗнаний)
	...
	Справочники.БЗ_СтатьиБазыЗнаний.ЗавершитьРедактированиеСтатьи(СтатьяБазыЗнаний);
...
Процедура ЗаписатьСтатьюНаСервере(ОсвободитьСтатью) 
	...
	Если ЗначениеЗаполнено(ЭтотОбъект.СтатьяБазыЗнаний) Тогда
		ЭлементОбъект = ЭтотОбъект.СтатьяБазыЗнаний.ПолучитьОбъект();
	Иначе 
		ЭлементОбъект = Справочники.БЗ_СтатьиБазыЗнаний.СоздатьЭлемент();
	КонецЕсли;
	...
	ЭлементОбъект.ДополнительныеСвойства.Вставить("БЗ_ОписаниеИзменений", БЗ_ОписаниеИзменений);
	// Общие данные
	...
	//+++ АйТи Суворова С. О. 06.02.2023 ТЗ № НФ00-003414 >> e1cib/data/Документ.ЗаданиеНаРаботу?ref=a935fc3497141b3f11ed82a9ee59bafc	Если ЗначениеЗаполнено(ЭтотОбъект.СтатусСтатьи) Тогда
		ЭлементОбъект.СтатусСтатьи = ЭтотОбъект.СтатусСтатьи;
	КонецЕсли;	//--- АйТи Суворова С. О. 06.02.2023 ТЗ № НФ00-003414 >> e1cib/data/Документ.ЗаданиеНаРаботу?ref=a935fc3497141b3f11ed82a9ee59bafc
	...
	ВыбраннаяСтатья = СтатьяБазыЗнаний; //+++АйТи Смирнов Д. 01.11.2021
	
	Если ПараметрыСеанса.ТекущийПользователь = ВыбраннаяСтатья.Ответственный Тогда
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	&СтатьяБазыЗнаний КАК СтатьяБазыЗнаний,
	|	Пользователи.Ссылка КАК Пользователь,
	|	БЗ_СостояниеСтатейБазыЗнаний.ВыданаНаИзучение КАК ВыданаНаИзучение,
	|	БЗ_СостояниеСтатейБазыЗнаний.БылаОткрыта КАК БылаОткрыта,
	|	БЗ_СостояниеСтатейБазыЗнаний.Изучена КАК Изучена,
	|	БЗ_СостояниеСтатейБазыЗнаний.ПровереноИзучение КАК ПровереноИзучение
	|ИЗ
	|	Справочник.Пользователи КАК Пользователи
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.БЗ_СостояниеСтатейБазыЗнаний КАК БЗ_СостояниеСтатейБазыЗнаний
	|		ПО Пользователи.Ссылка = БЗ_СостояниеСтатейБазыЗнаний.Пользователь
	|			И (БЗ_СостояниеСтатейБазыЗнаний.СтатьяБазыЗнаний = &СтатьяБазыЗнаний)
	|ГДЕ
	|	Пользователи.Ссылка В(&СписокПользователей)";
	
	Запрос.УстановитьПараметр("СтатьяБазыЗнаний", СтатьяБазыЗнаний);
	Запрос.УстановитьПараметр("СписокПользователей", СписокПользователейДляОповещания.ВыгрузитьЗначения());
	
	РезультатЗапроса = Запрос.Выполнить();
	
	НЗ = РегистрыСведений.БЗ_СостояниеСтатейБазыЗнаний.СоздатьНаборЗаписей();
	НЗ.Отбор.СтатьяБазыЗнаний.Установить(СтатьяБазыЗнаний);
	НЗ.Загрузить(РезультатЗапроса.Выгрузить());
	
	ТекПользователь = ПараметрыСеанса.ТекущийПользователь;
	
	Для каждого ЭлементНабораЗаписей Из НЗ Цикл
		Если НЕ ЗначениеЗаполнено(ЭлементНабораЗаписей.ВыданаНаИзучение) ИЛИ ВыдатьНаИзучениеЗаново Тогда
			ЭлементНабораЗаписей.ВыдалНаИзучение = ТекПользователь;
			ЭлементНабораЗаписей.ВыданаНаИзучение = ТекущаяДата();
			
			//+++ АйТи: [Кашин Константин]
			//Если ЭлементНабораЗаписей.Пользователь <> ТекПользователь Тогда
			//	АйТи_ОбщиеПроцедурыИПодписки.ОповеститьОНовыхСтатьяхБазыЗнаний(ЭлементНабораЗаписей.Пользователь,
			//		СтатьяБазыЗнаний, ЭлементНабораЗаписей.СрокИзучения, ТекущаяДата());
			//КонецЕсли;
		Иначе
			ЭлементНабораЗаписей.ВыдалНаИзучение = ТекПользователь;
		КонецЕсли;
		
		Если ВыдатьНаИзучениеЗаново Тогда
			ЭлементНабораЗаписей.БылаОткрыта = Дата(1,1,1);
			ЭлементНабораЗаписей.Изучена = Дата(1,1,1);
			ЭлементНабораЗаписей.ПровереноИзучение = Дата(1,1,1);
		КонецЕсли;
	КонецЦикла;
...
&НаСервере
Функция ПолучитьСтруктуруКартинок()
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	БЗ_ХранилищеДанныхБазыЗнаний.Идентификатор КАК Идентификатор,
		|	БЗ_ХранилищеДанныхБазыЗнаний.ХранилищеЗначения КАК ХранилищеЗначения,
		|	СтатьиБазыЗнанийКартинки.Имя КАК Имя
		|ИЗ
		|	Справочник.БЗ_СтатьиБазыЗнаний.Картинки КАК СтатьиБазыЗнанийКартинки
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.БЗ_ХранилищеДанныхБазыЗнаний КАК БЗ_ХранилищеДанныхБазыЗнаний
		|		ПО СтатьиБазыЗнанийКартинки.Ссылка = БЗ_ХранилищеДанныхБазыЗнаний.СтатьяБазыЗнаний
		|			И СтатьиБазыЗнанийКартинки.Идентификатор = БЗ_ХранилищеДанныхБазыЗнаний.Идентификатор
		|ГДЕ
		|	БЗ_ХранилищеДанныхБазыЗнаний.СтатьяБазыЗнаний = &Ссылка
		|	И БЗ_ХранилищеДанныхБазыЗнаний.ВидДанных = 1
		|	И СтатьиБазыЗнанийКартинки.Имя ПОДОБНО ""src_image%""";
	
	Запрос.УстановитьПараметр("Ссылка", СтатьяБазыЗнаний);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		ДанныеКартинки = ВыборкаДетальныеЗаписи.ХранилищеЗначения.Получить();
		СтруктураВозврата.Вставить(ВыборкаДетальныеЗаписи.Имя, Новый Картинка(ДанныеКартинки));
	КонецЦикла;
...
Процедура ОткрытьРазделСтатьи(знач ИдентификаторРаздела)
	...
	ОткрытьФорму("Справочник.БЗ_СтатьиБазыЗнаний.Форма.РедактированиеСтатьи", ПараметрыФормы, ЭтотОбъект, , , , ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ВыполнитьВставитьСноску(Результат, ДопПараметры) Экспорт
	...
	ОткрытьФорму("Справочник.БЗ_СтатьиБазыЗнаний.Форма.РедактированиеСноски",
		ПараметрыФормы,
		ЭтотОбъект, , , ,
		ОписаниеОповещения,
...
Процедура ПриОкончанииРедактированияСноски(Результат, ДопПараметры) Экспорт 
	...
	//+++ АйТи КучеровРМ 02.08.2018 ТЗ № ИС00-002921
	//Причина заменты кода: вылетает при ТекстСтатьи.Вставить
	//+++Было до 02.08.2018 14:52:14 // Далее тут старый код, который теперь заменён
...
Процедура ВыполнитьВставитьГиперссылку(Результат, ДопПараметры) Экспорт 
	...
	ОписаниеСсылки = БЗ_БазаЗнанийКлиентСервер.ПолучитьСтруктуруСсылкиПоАдресу(ДопПараметры.Гиперссылка);
	...
	ОткрытьФорму("Справочник.БЗ_СтатьиБазыЗнаний.Форма.РедактированиеГиперссылки",
		ПараметрыФормы,
		ЭтотОбъект, , , ,
		ОписаниеОповещения,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ПриОкончанииРедактированияГиперссылки(Результат, ДопПараметры) Экспорт 
	...
	Если Результат.Удалить Тогда
		НавигационнаяСсылка = "";
	Иначе 
		НавигационнаяСсылка = БЗ_БазаЗнанийКлиентСервер.ПолучитьАдресСсылкиПоСтруктуре(Результат);
	КонецЕсли;
	
	РезультатВставки = ТекстСтатьи.Вставить(ДопПараметры.Начало, Результат.Представление);
	//+++ АйТи КучеровРМ 02.08.2018 ТЗ № ИС00-002921
	//Причина заменты кода: вылетает при ТекстСтатьи.Вставить 
	//+++Было до 02.08.2018 14:48:52 // Далее тут старый код, который теперь заменён
...
Процедура ВыполнитьВставитьКартинку(Результат, ДопПараметры) Экспорт
	...
	ОткрытьФорму("Справочник.БЗ_СтатьиБазыЗнаний.Форма.РедактированиеКартинки",
		ПараметрыФормы,
		ЭтотОбъект, , , ,
		ОписаниеОповещения,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ПриОкончанииРедактированияКартинки(Результат, ДопПараметры) Экспорт
	...
	Если БЗ_БазаЗнанийКлиентСервер.ЭтоСтрокаКартинки(ДопПараметры.Текст) Тогда
		ТекстСтатьи.Удалить(ДопПараметры.Начало, ДопПараметры.Конец);
	КонецЕсли;
	...
	//+++ АйТи КучеровРМ 02.08.2018 ТЗ № ИС00-002921
	//Причина заменты кода: вылетает при ТекстСтатьи.Вставить
	//+++Было до 02.08.2018 14:43:53 // Далее тут старый код, который теперь заменён
	//РезультатВставки = ТекстСтатьи.Вставить(ДопПараметры.Начало, ТекстПодстановки);
	//РезультатВставки.ЦветФона = WebЦвета.БледноЗеленый;
	//
	//НачалоВыделения	= РезультатВставки.ЗакладкаНачала;
	//
	////Затем вставляется пробел и красится в белый, чтобы продолжить номарльно печатать
	//РезультатВставки = ТекстСтатьи.Вставить(РезультатВставки.ЗакладкаКонца, " ");
	//РезультатВставки.ЦветФона = Элементы.ТекстСтатьи.ЦветФона;
	//
	//Элементы.ТекстСтатьи.УстановитьГраницыВыделения(НачалоВыделения, РезультатВставки.ЗакладкаКонца);
	//---Было до 02.08.2018 14:43:53
	//Айти_ОбщиеФункцииКлиент.ТекстСтатьиВставить(ТекстСтатьи, Элементы.ТекстСтатьи, ДопПараметры.Начало, ТекстПодстановки); 
	//--- АйТи КучеровРМ 02.08.2018 ТЗ № ИС00-002921 
...
Функция ПолучитьАдресХранилищаКартинкиНаСервере(ИдентификаторФормы, СтатьяБазыЗнаний, Область, Идентификатор)
	...
	|	РегистрСведений.БЗ_ХранилищеДанныхБазыЗнаний КАК БЗ_ХранилищеДанныхБазыЗнаний
	|ГДЕ
	|	БЗ_ХранилищеДанныхБазыЗнаний.СтатьяБазыЗнаний = &СтатьяБазыЗнаний
	|	И БЗ_ХранилищеДанныхБазыЗнаний.Область = &Область
	|	И БЗ_ХранилищеДанныхБазыЗнаний.Идентификатор = &Идентификатор";
...
Процедура ВыполнитьВставитьТаблицу(Результат, ДопПараметры) Экспорт
	...
	ОткрытьФорму("Справочник.БЗ_СтатьиБазыЗнаний.Форма.РедактированиеТаблицы",
		ПараметрыФормы,
		ЭтотОбъект, , , ,
		ОписаниеОповещения,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ПриОкончанииРедактированияТаблицы(Результат, ДопПараметры) Экспорт
	...
	Если БЗ_БазаЗнанийКлиентСервер.ЭтоСтрокаТаблицы(ДопПараметры.Текст) Тогда
		ТекстСтатьи.Удалить(ДопПараметры.Начало, ДопПараметры.Конец);
	КонецЕсли;
	...
	//+++ АйТи КучеровРМ 02.08.2018 ТЗ № ИС00-002921
	//Причина заменты кода: вылетает при ТекстСтатьи.Вставить
	//+++Было до 02.08.2018 14:51:27 // Далее тут старый код, который теперь заменён
...
Функция ПолучитьДанныеВыбораДляКлючевогоПоля(Текст, ИндексВСписке)
	...
	|	БЗ_КлючевыеСловаБазыЗнаний.Ссылка КАК Тег,
	|	ПРЕДСТАВЛЕНИЕ(БЗ_КлючевыеСловаБазыЗнаний.Ссылка) КАК Представление,
	|	ЕСТЬNULL(БЗ_РейтингКлючевыхСловБазыЗнаний.Количество, 0) КАК Рейтинг
	|ИЗ
	|	Справочник.БЗ_КлючевыеСловаБазыЗнаний КАК БЗ_КлючевыеСловаБазыЗнаний
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.БЗ_РейтингКлючевыхСловБазыЗнаний КАК БЗ_РейтингКлючевыхСловБазыЗнаний
	|		ПО БЗ_КлючевыеСловаБазыЗнаний.Ссылка = БЗ_РейтингКлючевыхСловБазыЗнаний.КлючевоеСлово
	|			И (БЗ_РейтингКлючевыхСловБазыЗнаний.КатегорияБазыЗнаний = ЗНАЧЕНИЕ(Справочник.БЗ_КатегорииБазыЗнаний.ПустаяСсылка))
	|ГДЕ
	|	БЗ_КлючевыеСловаБазыЗнаний.Наименование >= &ВведеннаяСтрока
	|	И ПОДСТРОКА(БЗ_КлючевыеСловаБазыЗнаний.Наименование, 1, 500) = &ВведеннаяСтрока
	|
	|УПОРЯДОЧИТЬ ПО
	|	БЗ_КлючевыеСловаБазыЗнаний.Наименование,
	|	Рейтинг";
...
Функция РазобратьСтрокуСТегами(СтрокаТегов)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	БЗ_КлючевыеСловаБазыЗнаний.Ссылка,
	|	БЗ_КлючевыеСловаБазыЗнаний.Наименование
	|ИЗ
	|	Справочник.БЗ_КлючевыеСловаБазыЗнаний КАК БЗ_КлючевыеСловаБазыЗнаний
	|ГДЕ
	|	БЗ_КлючевыеСловаБазыЗнаний.Наименование В(&МассивНаименований)
	|	И НЕ БЗ_КлючевыеСловаБазыЗнаний.ПометкаУдаления";
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
...
//Добавление регистратора, признак - Статья Изучена

&НаСервереБезКонтекста
Процедура УстановитьПризнакИзученостиСтатьи(ВыбраннаяСтатья)  //+++АйТи Смирнов Д. 01.11.2021
...
Процедура ПрикрепленныеФайлы(Команда)
	...
	Если НЕ ЗначениеЗаполнено(СтатьяБазыЗнаний) Тогда
		Сообщить("Запишите сначала статью");
		Возврат;
	КонецЕсли;
	
	ПараметрКоманды = СтатьяБазыЗнаний;
	...
	ОткрытьФорму("Обработка.БЗ_РаботаСФайлами.Форма.ПрисоединенныеФайлы", ПараметрыФормы, ЭтаФорма, Истина, Неопределено); //ОткрытьФорму("ОбщаяФорма.ПрисоединенныеФайлы", ПараметрыФормы, ЭтаФорма, Истина, Неопределено);

	
КонецПроцедуры

&НаКлиенте
Процедура ВставитьКартинкуБыстро(Команда)
	
	// Доп параметры вставки
	ВыделенныйТекст	= СокрЛП(Элементы.ТекстСтатьи.ВыделенныйТекст);
	ЗакладкаНачало	= Неопределено;
	ЗакладкаКонец 	= Неопределено;
	Элементы.ТекстСтатьи.ПолучитьГраницыВыделения(ЗакладкаНачало, ЗакладкаКонец);
	...
	ИмяКартинки		= БЗ_БазаЗнанийКлиентСервер.ПолучитьИмяКартинки(ВыделенныйТекст);
	ДопПараметры	= Новый Структура("ИмяКартинки, Текст, Начало, Конец", ИмяКартинки, ВыделенныйТекст, ЗакладкаНачало, ЗакладкаКонец);
	ДопПараметры.Вставить("ИдентификаторКартинки", Строка(Новый УникальныйИдентификатор));
	...
	Если ДиалогВыбораФайла.Выбрать() Тогда
		...
		ТекДата = ТекущаяДата();
		Имя 	= "Picture_" + Формат(ТекДата, "ДФ=yyyy_MM_dd_HH_mm_ss"); //Файл.ИмяБезРасширения;
		
		ЭтотОбъект.Модифицированность = Истина;
		ЭтотОбъект.ОбновитьОтображениеДанных();
	КонецЕсли;
	
	Если ПустаяСтрока(ДиалогВыбораФайла.ПолноеИмяФайла) Тогда
		Возврат;	
	КонецЕсли;
	...
	лВысотаКартинки 	= ВысотаКартинкиЧерезInternetExplorer(ДиалогВыбораФайла.ПолноеИмяФайла); 
	Если лВысотаКартинки > 500 Тогда
		лВысотаКартинки = 500;		
	КонецЕсли; 
	
	// Выбрать
	Результат = БЗ_БазаЗнанийКлиентСервер.ПроверитьПравильностьЗаполненияИмени(Имя);
	Если НЕ Результат Тогда
	...
	ПараметрыЗакрытия.Вставить("Выравнивание"		, Выравнивание);
	ПараметрыЗакрытия.Вставить("Высота"				, лВысотаКартинки);
	ПараметрыЗакрытия.Вставить("Граница"			, 0);
	ПараметрыЗакрытия.Вставить("Имя"				, Имя);
	ПараметрыЗакрытия.Вставить("Подпись"			, "");
	ПараметрыЗакрытия.Вставить("Подсказка"			, "");
	ПараметрыЗакрытия.Вставить("Модифицированность"	, Истина);
	
	ПриОкончанииРедактированияКартинки(ПараметрыЗакрытия, ДопПараметры);
...
Процедура ПроверкаПравописания(Команда)
	...
	Если ПустаяСтрока(ТекстСтатьи.ПолучитьТекст())Тогда Возврат; КонецЕсли;
	лТекст  = "";
	лТекстВложения   = Новый Структура;
	ТекстСтатьи.ПолучитьHTML(лТекст, лТекстВложения);
	БЗ_ОбщегоНазначенияКлиент.ПроверитьПравописаниеТекста(лТекст, лТекстВложения,  "ТекстСтатьи", ЭтотОбъект); //Вернётся через Оповестить: ПроверкаПравописания_ИзмененТекст	
	
КонецПроцедуры

&НаКлиенте
Процедура ПодборПользователейДляОповещения(Команда)
	
	ПараметрыОткрытияФормы = Новый Структура;
	ПараметрыОткрытияФормы.Вставить("Выбранные", СписокПользователейДляОповещания.ВыгрузитьЗначения());
...
Процедура ПослеПодбораПользователейДляОповещения(Результат, ДопПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		СписокПользователейДляОповещания.ЗагрузитьЗначения(Результат);
		Модифицированность = Истина;
	КонецЕсли;
...
&НаКлиенте
Функция ВысотаКартинкиЧерезInternetExplorer(ПутьКФайлу = "")
	...
	Если ФайлНаДиске.Существует() Тогда
		лФайлНаДискеРасширение 	= НРег(ФайлНаДиске.Расширение); 
		Если лФайлНаДискеРасширение = ".jpg" Тогда // иначе при png ИЕ.Navigate(ПутьКФайлу) попадает в загрузку файла и не получает ("IMG").item(0)
			
			Попытка
				
				ИЕ 				= Новый COMОбъект("InternetExplorer.Application");
				ИЕ.Fullscreen 	= -1;
				ИЕ.Navigate(ПутьКФайлу);
				IMG 		= ИЕ.document.getElementsByTagName("IMG").item(0);
				//Сообщить(IMG.Height);
				//Сообщить(IMG.Width);
				лРезультат 	= IMG.Height;
				
			Исключение
				Сообщить("Ошибка при установке размера картинки, поэтому установлен актуальный размер. Ошибка: " + ОписаниеОшибки(), СтатусСообщения.Внимание);
			КонецПопытки;
...
&НаСервере
Процедура ЗагрузитьСписокПользователейДляОповещания()
	...
	РезультатЗапроса = Запрос.Выполнить();
	СписокПользователейДляОповещания.ЗагрузитьЗначения(РезультатЗапроса.Выгрузить().ВыгрузитьКолонку("Пользователь"));
	
КонецПроцедуры // ЗагрузитьСписокПользователейДляОповещания()  

&НаСервере
// Процедура обновляет значение поля "ТекстСтатьи".
Процедура ОбновитьТекстСтатьиHTML(Текст, Вложения)	
	ТекстСтатьи.УстановитьHTML(Текст, Вложения); //УстановитьHTML можно только &НаСервере
	//ТекстСтатьи.ПолучитьHTML(КомментарийОформлениеПредставление, Новый Структура); //Обновляем представление HTML	
КонецПроцедуры         

&НаКлиенте
Процедура ПередЗаписью(ЗакрытьФорму = Ложь, ДопПараметры = Неопределено, ОписаниеИзмененийУказано = Ложь)
	
	лНовый = СтатьяБазыЗнаний.Пустая();
	ТекстСтатьиИзм	= ТекстСтатьиИзменился();
	
	Если НЕ лНовый И ТекстСтатьиИзм И НЕ ОписаниеИзмененийУказано И НЕ ЭтоПодраздел Тогда
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ВыдатьНаИзучениеЗаново", ВыдатьНаИзучениеЗаново);
		ПараметрыФормы.Вставить("БЗ_ОписаниеИзменений", БЗ_ОписаниеИзменений);
		
		ДопПараметрыОповещения = Новый Структура;
		ДопПараметрыОповещения.Вставить("ЗакрытьФорму", ЗакрытьФорму);
		ДопПараметрыОповещения.Вставить("ДопПараметры", ДопПараметры);
		
		ОП = Новый ОписаниеОповещения("ПослеУказанияОписаниеИзменений", ЭтаФорма, ДопПараметрыОповещения);
		
		ОткрытьФорму("Справочник.БЗ_СтатьиБазыЗнаний.Форма.ВнесениеИзмененийВСтатью", ПараметрыФормы, ЭтаФорма,,,, ОП, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		
		Возврат;
	КонецЕсли;
	
	ПреобразоватьНовыеКартинкиИзBase64();
	Если ТекстСтатьиИзм Тогда
		ПреобразованиеГиперссылок();
	КонецЕсли;
	...
	// Произошла запись статьи или подраздела
	Если ПараметрыЗакрытия.Количество() = 0 Тогда // оповещение о записи статьи или подраздела
		//+++ АйТи Кучеров Р.М. 09.10.2020 ТЗ № ИС00-001146 >> e1cib/data/Документ.ЗаданиеНаРаботу?ref=a17f32336337376211e9249202ccc0a0
		//Оповестить("СтатьиБазыЗнанийЗапись", , СтатьяБазыЗнаний);
		лПараметры = Новый Структура("Новый", лНовый);
		Оповестить("СтатьиБазыЗнанийЗапись", лПараметры, СтатьяБазыЗнаний);;
		//--- АйТи Кучеров Р.М. 09.10.2020 ТЗ № ИС00-001146 >> e1cib/data/Документ.ЗаданиеНаРаботу?ref=a17f32336337376211e9249202ccc0a0
	Иначе // оповещение владельца о необходимости записи данных формы
		Если ТипЗнч(ЭтотОбъект.ВладелецФормы) = Тип("УправляемаяФорма") И ЭтотОбъект.ВладелецФормы.ИмяФормы = "Справочник.БЗ_СтатьиБазыЗнаний.Форма.РедактированиеСтатьи" Тогда
			ЭтотОбъект.ВладелецФормы.ПриОкончанииРедактированияПодраздела(ПараметрыЗакрытия, Неопределено);
			ЭтотОбъект.ВладелецФормы.ЗаписатьНаСервере(Ложь);
		КонецЕсли;
	КонецЕсли;
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Привязки, "Статья", СтатьяБазыЗнаний, ВидСравненияКомпоновкиДанных.Равно,,Истина,РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный);
		ИнициализацияДанных(Ложь);
	КонецЕсли;
	
КонецПроцедуры   

&НаСервере
Функция ТекстСтатьиИзменился()
	
	Значение = ТекстСтатьи.ПолучитьТекст();
	...
	Возврат СтатьяБазыЗнаний.ПолныйТекст <> Значение;
	
КонецФункции

&НаСервере
Процедура ПреобразоватьНовыеКартинкиИзBase64()
	
	ПоследнийНумераторКартинок = 0;
	
	//Для каждого ЭлементКартинка Из Картинки Цикл
	//	Если СтрНайти(ЭлементКартинка.Имя, "src-image") > 0 Тогда
	//		Нумератор = Число(СтрЗаменить(ЭлементКартинка.Имя, "src-image", ""));
	//		ПоследнийНумераторКартинок = Макс(Нумератор, ПоследнийНумераторКартинок);
	//	КонецЕсли;
	//КонецЦикла;
	
	МассивУдаления = Новый Массив;
	Для каждого ЭлементКартинка Из Картинки Цикл
		Если СтрНайти(ЭлементКартинка.Имя, "src_image") > 0 Тогда
			МассивУдаления.Добавить(ЭлементКартинка);
		КонецЕсли;
	КонецЦикла;
	
	Для каждого ЭлементМассива Из МассивУдаления Цикл
		Картинки.Удалить(ЭлементМассива);
	КонецЦикла;
	
	Для каждого ЭлементДокумента из ТекстСтатьи.ПолучитьЭлементы() Цикл
		Если Тип(ЭлементДокумента) = Тип("КартинкаФорматированногоДокумента") Тогда
			Если Найти(ЭлементДокумента.Картинка,"base64")> 0 тогда
				
				СтрокаКартинка = ЭлементДокумента.Картинка;
				...
				БезНачала = Сред(СтрокаКартинка,Найти(СтрокаКартинка, "base64,")+СтрДлина("base64,"));
				...
				ДанныеКартинки = Base64Значение(БезНачала);
			ИначеЕсли ТипЗнч(ЭлементДокумента.Картинка) = Тип("Картинка") Тогда
				ДанныеКартинки = ЭлементДокумента.Картинка.ПолучитьДвоичныеДанные();
			Иначе
				Продолжить;
			КонецЕсли; 
			
			АдресХранилища = ПоместитьВоВременноеХранилище(ДанныеКартинки, Идентификатор);
			
			ПоследнийНумераторКартинок = ПоследнийНумераторКартинок + 1;
			
			СтрокаТаблицы = ЭтотОбъект.Картинки.Добавить();
			СтрокаТаблицы.Область				= ЭтотОбъект.Область;
			СтрокаТаблицы.ИдентификаторКартинки	= Новый УникальныйИдентификатор;
			СтрокаТаблицы.Имя					= "src_image" + Формат(ПоследнийНумераторКартинок, "ЧГ=");
			СтрокаТаблицы.Подпись				= СтрШаблон("src=""%1""", СтрокаТаблицы.Имя);
			СтрокаТаблицы.АдресХранилища		= АдресХранилища;
			
			ШиринаКартинки = ЭлементДокумента.Ширина;
			ВысотаКартинки = ЭлементДокумента.Высота;
			
			ЭлементДокумента.Картинка = СтрокаТаблицы.Имя;
			
			ЭлементДокумента.Ширина = ШиринаКартинки;
			ЭлементДокумента.Высота = ВысотаКартинки;
		КонецЕсли;        
	КонецЦикла;
...
&НаСервере
Процедура ПреобразованиеГиперссылок()
	
	МассивУдаления = Новый Массив;
	
	ЭлементыСтатьи = ТекстСтатьи.ПолучитьЭлементы();
	Для каждого ЭлементСписка Из ЭлементыСтатьи Цикл
		Если ТипЗнч(ЭлементСписка) = Тип("ТекстФорматированногоДокумента") И НЕ ЗначениеЗаполнено(ЭлементСписка.НавигационнаяСсылка) Тогда
			РезультатПоиска = БЗ_ОбщиеФункцииСервер.СтруктураИТипСсылкиИзТекста(ЭлементСписка.Текст);
			Если ЗначениеЗаполнено(РезультатПоиска) Тогда
				Если РезультатПоиска.Свойство("ТекстДо") ИЛИ РезультатПоиска.Свойство("ТекстПосле") Тогда
					ЗакладкаНачала = ЭлементСписка.ЗакладкаНачала;
					ЗакладкаКонца = ЭлементСписка.ЗакладкаКонца;
					
					ЦветТекста = ЭлементСписка.ЦветТекста;
					ЦветФона = ЭлементСписка.ЦветФона;
					Шрифт = ЭлементСписка.Шрифт;
					
					ТекстСтатьи.Удалить(ЭлементСписка.ЗакладкаНачала, ЭлементСписка.ЗакладкаКонца);
					
					Если РезультатПоиска.Свойство("ТекстДо") Тогда
						РезультатВставки = ТекстСтатьи.Вставить(ЗакладкаНачала, РезультатПоиска.ТекстДо + " ");
						РезультатВставки.ЦветТекста = ЦветТекста;
						РезультатВставки.ЦветФона = ЦветФона;
						РезультатВставки.Шрифт = Шрифт;
						
						ЗакладкаНачала = РезультатВставки.ЗакладкаКонца;
					КонецЕсли;
					
					РезультатВставки = ТекстСтатьи.Вставить(ЗакладкаНачала, РезультатПоиска.ТекстСсылки);
					РезультатВставки.ЦветТекста = ЦветТекста;
					РезультатВставки.ЦветФона = ЦветФона;
					РезультатВставки.Шрифт = Шрифт;
					
					РезультатВставки.НавигационнаяСсылка = РезультатПоиска.ТекстСсылки;
					РезультатВставки.ЦветТекста = WebЦвета.Синий;
					
					Если РезультатПоиска.Свойство("ПредставлениеСсылки") Тогда
						РезультатВставки.Текст = РезультатПоиска.ПредставлениеСсылки;
					КонецЕсли;
					
					ЗакладкаНачала = РезультатВставки.ЗакладкаКонца;
					
					Если РезультатПоиска.Свойство("ТекстПосле") Тогда
						РезультатВставки = ТекстСтатьи.Вставить(ЗакладкаНачала, " " + РезультатПоиска.ТекстПосле);
						РезультатВставки.ЦветТекста = ЦветТекста;
						РезультатВставки.ЦветФона = ЦветФона;
						РезультатВставки.Шрифт = Шрифт;
					...
					КонецЕсли;
				Иначе
					ЭлементСписка.НавигационнаяСсылка = РезультатПоиска.ТекстСсылки;
					ЭлементСписка.ЦветТекста = WebЦвета.Синий;
					
					Если РезультатПоиска.Свойство("ПредставлениеСсылки") Тогда
						ЭлементСписка.Текст = РезультатПоиска.ПредставлениеСсылки;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;                                      
	...
	//Для каждого ЭлементМассива Из МассивУдаления Цикл
	//	ТекстСтатьи.Удалить(ЭлементМассива.ЗакладкаНачала, ЭлементМассива.ЗакладкаКонца);
	//КонецЦикла;
	
КонецПроцедуры

&НаКлиенте
Процедура ПослеУказанияОписаниеИзменений(Результат, ДопПараметры) Экспорт
	
	//+++ АйТи ШариковаТВ от 06.05.2022 ТЗ№ ИС00-002851 e1cib/data/Документ.ЗаданиеНаРаботу?ref=a919fc3497141b3f11ecc4726253e9a5   
	//  Проверка нажата ли кнопка Отмены 
	//  Если да, то возвращаемся в форму редактирования статьи 
	Если Результат.Свойство( "НажатаОтмена" ) Тогда
		Возврат;
	КонецЕсли;
	//--- АйТи ШариковаТВ от 06.05.2022 ТЗ№ ИС00-002851 e1cib/data/Документ.ЗаданиеНаРаботу?ref=a919fc3497141b3f11ecc4726253e9a5	
	
	Если Результат <> Неопределено Тогда
		ВыдатьНаИзучениеЗаново = Результат.ВыдатьНаИзучениеЗаново;
		ОписаниеИзменений = Результат.БЗ_ОписаниеИзменений;
	КонецЕсли;
	
	ПередЗаписью(ДопПараметры.ЗакрытьФорму, ДопПараметры.ДопПараметры, Истина);
	
КонецПроцедуры


&НаКлиенте
Процедура СписокПользователейДляОповещанияЗначениеПриИзменении(Элемент)
	Модифицированность = Истина;
КонецПроцедуры

//+++ АйТи Суворова С. О. 06.02.2023 ТЗ № НФ00-003414 >> e1cib/data/Документ.ЗаданиеНаРаботу?ref=a935fc3497141b3f11ed82a9ee59bafc
&НаКлиенте
Процедура ОткрытьВсплывающуюГруппу(Команда)
	
	ИмяГруппы = "ВсплывающаяГруппа_" + СтрЗаменить(Команда.Имя, "_КомандаОткрытияГруппы", ""); 
	
	Если НЕ Элементы.Найти( ИмяГруппы ) = Неопределено Тогда
		Элементы[ИмяГруппы].Показать();
	КонецЕсли;
	
КонецПроцедуры //--- АйТи Суворова С. О. 06.02.2023 ТЗ № НФ00-003414 >> e1cib/data/Документ.ЗаданиеНаРаботу?ref=a935fc3497141b3f11ed82a9ee59bafc

&НаКлиенте
Процедура СтатусСтатьиПриИзменении(Элемент)
	Модифицированность = Истина;
КонецПроцедуры
...
Процедура ЗаписатьОповещениеПользователей()
	...
	ЭтоНовый = ЭтотОбъект.СтатьяБазыЗнаний.Ссылка.Пустая();
	Если НЕ ЭтоНовый и не ВыдатьНаИзучениеЗаново Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого ЭлементМассива Из Получатели Цикл
		...
		Если ВыдатьНаИзучениеЗаново Тогда
			СтрОписание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='В базе знаний изменена статья: %1'"),
				Строка(ЭтотОбъект.СтатьяБазыЗнаний));
			ВидСобытия = 2;
		Иначе
			СтрОписание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='В базе знаний появилась новая статья: %1'"),
				Строка(ЭтотОбъект.СтатьяБазыЗнаний));
			ВидСобытия = 1;
		КонецЕсли;
		...
		ЗаписьРегистра.Объект			= ЭтотОбъект.СтатьяБазыЗнаний;
...
Функция ПолучитьПользователейДляОповещения()
	...
	Возврат ЭтотОбъект.СписокПользователейДляОповещания.ВыгрузитьЗначения();
