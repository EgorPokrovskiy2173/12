...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПапкаПриОткрытии = Справочники.БЗ_ПапкиФайлов.ПустаяСсылка() Или ПапкаПриОткрытии = Неопределено Тогда
		ПапкаПриОткрытии = Справочники.БЗ_ПапкиФайлов.Шаблоны;
	КонецЕсли;
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ВРег(ИсточникВыбора.ИмяФормы) = ВРег("Справочник.БЗ_ПапкиФайлов.Форма.ФормаВыбора") Тогда
...
Процедура СоздатьПапкуВыполнить()
	...
	ОткрытьФорму("Справочник.БЗ_ПапкиФайлов.ФормаОбъекта", ПараметрыСозданияПапки, Элементы.Папки);
...
Процедура ЗакончитьРедактирование(Команда)
	...
	Если МассивФайлов.Количество() > 1 Тогда
		...
		ОткрытьФорму("Обработка.БЗ_РаботаСФайлами.Форма.ФормаЗавершениеРедактирования", ПараметрыФормы, ЭтотОбъект);
...
Процедура СписокПередНачаломИзменения(Элемент, Отказ)
	...
	ОткрытьФорму("Справочник.БЗ_Файлы.ФормаОбъекта", ПараметрыОткрытияФормы);
...
Процедура ПеренестиВПапку(Команда)
	...
	ОткрытьФорму("Справочник.БЗ_ПапкиФайлов.ФормаВыбора", ПараметрыФормы, ЭтотОбъект);
...
Процедура ИмпортФайловПослеУстановкиРасширения(Результат, ПараметрыВыполнения) Экспорт
	...
	ОткрытьФорму("Обработка.БЗ_РаботаСФайлами.Форма.ФормаИмпортаФайлов", ПараметрыФормы);
КонецПроцедуры
...
Процедура ИмпортПапкиПослеУстановкиРасширения(Результат, ПараметрыВыполнения) Экспорт
	...
	ОткрытьФорму("Обработка.БЗ_РаботаСФайлами.Форма.ФормаИмпортаПапки", ПараметрыФормы);
...
Процедура ЭкспортПапкиПослеУстановкиРасширения(Результат, ПараметрыФормы) Экспорт
	...
	ОткрытьФорму("Обработка.БЗ_РаботаСФайлами.Форма.ФормаЭкспортаПапки", ПараметрыФормы);
...
Процедура ПеретаскиваниеВПапку(ПапкаДляДобавления, ЗначениеПеретаскивания, Действие)
	...
	ИначеЕсли ТипЗнч(ЗначениеПеретаскивания) = Тип("Массив") Тогда
		...
		ИначеЕсли ТипЗначения = Тип("СправочникСсылка.БЗ_Файлы") Тогда
			Если ПапкаДляДобавления.Пустая() Тогда
				Возврат;
			КонецЕсли;
		...
		ИначеЕсли ТипЗначения = Тип("СправочникСсылка.БЗ_ПапкиФайлов") Тогда
...
Функция ПараметрыНастройкиСинхронизации(ВладелецФайла)
	...
	ТипВладельцаФайла = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(Тип("СправочникСсылка.БЗ_Файлы"));
