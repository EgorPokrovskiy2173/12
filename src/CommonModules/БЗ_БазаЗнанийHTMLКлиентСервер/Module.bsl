...
Функция ПолучитьОбщуюЧастьСтраницы(Параметры)
	...
	ТаблицаСтилей	= БЗ_БазаЗнанийCSSКлиентСервер.ПолучитьТаблицуСтилей();
	...
	|				Copyright © 2003-2099 <a href='http://www.itcons76.ru/'>ООО ""АйТи-Консалтинг""</a>.
	|				All rights reserved.
	|				Phone: +7(4852)59-3333. E-mail: <a href='mailto:info@allzon.ru'>Написать нам</a>
	|			</div>
	|		</div>	
	|	</body>
	|</html>";
...
Функция ПолучитьОбщиеФайлы() Экспорт
	...
	#Если ВебКлиент ИЛИ Сервер ИЛИ ВнешнееСоединение Тогда
		ВидХранения = "web";
		Каталог		= БЗ_БазаЗнанийКлиентСерверПовтИсп.ПолучитьЗначениеОбщейНастройки("WebКаталог");
		Каталог		= Каталог + ?(Прав(Каталог, 1) + "/", "", "/");
	#Иначе
		ВидХранения = "local";
		Каталог		= БЗ_БазаЗнанийКлиентСерверПовтИсп.ПолучитьЗначениеОбщейНастройки("ЛокальныйКаталог");
...
Функция ПолучитьПараметрыВыводаСтраницыПоВиду(ВидСтраницы) Экспорт
	...
	ИначеЕсли ВидСтраницы = "category_articles" Тогда
		...
		ПараметрыВывода.Порядок		= "Наименование ВОЗР";
	КонецЕсли;
...
Процедура ОбработатьОбщиеСкриптыЛокально(СтруктураСкриптов, ТекстHTML)
	...
	КонецЦикла;
	ТекстыСкриптов	= БЗ_БазаЗнанийВызовСервера.ПолучитьТекстыСкриптов(ФайлыЗагрузки);
...
Процедура ОбработатьОбщиеКартинкиЛокально(СтруктураФайлов)
	...
	#Если Клиент Тогда
		ПолучаемыеФайлы = БЗ_БазаЗнанийВызовСервера.ПоместитьОбщиеКартинкиВХранилище(ФайлыЗагрузки);
...
Функция ПолучитьОбщиеКартинки(ВидХранения, Каталог)
	...
	КаталогПоУмолчанию = СокрЛП(Каталог);
	СтруктураКартинок.Вставить("ПолучитьСсылку", ПолучитьСтруктуруДанныхФайла(КаталогПоУмолчанию, "it_link.png"));
	СтруктураКартинок.Вставить("ЗеленнаяГалка", ПолучитьСтруктуруДанныхФайла(КаталогПоУмолчанию, "it_jackdaw.png"));
	СтруктураКартинок.Вставить("КрасныйКрест", ПолучитьСтруктуруДанныхФайла(КаталогПоУмолчанию, "it_RedCross.png"));
	СтруктураКартинок.Вставить("ТребуетсяАктуализация", ПолучитьСтруктуруДанныхФайла(КаталогПоУмолчанию, "UpdatingIsRequired.png"));
	//+++ АйТи Чириков В. А. 22.03.2023 ТЗ № ИС00-001047 >> e1cib/data/Документ.ЗаданиеНаРаботу?ref=a93bfc3497141b3f11eda38f7dd40ec4
	СтруктураКартинок.Вставить("ЗвездаПустая", ПолучитьСтруктуруДанныхФайла(КаталогПоУмолчанию, "star_low.png"));
	//--- АйТи Чириков В. А. 22.03.2023 ТЗ № ИС00-001047 >> e1cib/data/Документ.ЗаданиеНаРаботу?ref=a93bfc3497141b3f11eda38f7dd40ec4
	
	Для Счетчик = 1 По 78 Цикл
		СтруктураКартинок.Вставить("TypeIconFile_" + Счетчик, ПолучитьСтруктуруДанныхФайла(КаталогПоУмолчанию, "TypeIconFile_" + Счетчик + ".png"));  
	КонецЦикла;
