...
Функция ОбработатьРезультатПолнотекстовогоПоиска(Текст1С, МассивЭлементов) Экспорт
	...
	Для Каждого СтатьяСсылка Из МассивЭлементов Цикл
		...
		Если ТипЗнч(СтатьяСсылка) = Тип("СправочникСсылка.БЗ_СтатьиБазыЗнаний") Тогда
	...
	ТекстСтраницы = "<html>
	|" + БЗ_БазаЗнанийCSSКлиентСервер.ПолучитьТаблицуСтилейПоиска() + "
	|<body>
	|" + ТекстРезультата + "
	|</body>
	|</html>";
...
Процедура ПередЗавершениемРаботыСистемы(Отказ) Экспорт
	...
	БЗ_БазаЗнанийВызовСервера.ПередЗавершениемРаботыСистемыНаСервере(Отказ);
