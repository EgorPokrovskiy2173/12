...
Функция ПроверитьПравильностьЗаполненияИмени(знач Имя) Экспорт
	...
	Если НЕ ПерваяЦифра Тогда
		ДопустимыеСимволы	= БЗ_БазаЗнанийКлиентСерверПовтИсп.ПолучитьДопустимыеСимволыИмени();
...
Функция ПолучитьСтруктуруСсылкиПоАдресу(знач АдресСсылки) Экспорт
	...
	ИначеЕсли Лев(АдресСсылки, 9) = "kb://api/" Тогда
		ДанныеСсылки = БЗ_БазаЗнанийAPIКлиентСервер.РазобратьАдресСсылки(АдресСсылки);
...
Функция ПолучитьАдресСсылкиПоСтруктуре(знач СтруктураСсылки) Экспорт
	...
	Если СтруктураСсылки.Тип = 0 Тогда
		...
		АдресСсылки = БЗ_БазаЗнанийAPIКлиентСервер.КонструкторСсылки_page("article", Параметры);
	...
	ИначеЕсли СтруктураСсылки.Тип = 3 Тогда
		...
		АдресСсылки = БЗ_БазаЗнанийAPIКлиентСервер.КонструкторСсылки_page("category", Параметры);
	ИначеЕсли СтруктураСсылки.Тип = 4 Тогда
		Параметры	= Новый Структура;
		Параметры.Вставить("text", СтруктураСсылки.Адрес);
		
		АдресСсылки = БЗ_БазаЗнанийAPIКлиентСервер.КонструкторСсылки_search("text", Параметры);
	ИначеЕсли СтруктураСсылки.Тип = 5 Тогда
		СтрокаПоиска	= БЗ_БазаЗнанийВызовСервера.ЗакодироватьСтроку(СтруктураСсылки.Адрес);
