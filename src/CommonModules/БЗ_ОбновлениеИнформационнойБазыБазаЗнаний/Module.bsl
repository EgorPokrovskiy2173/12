...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура           = "БЗ_ОбновлениеИнформационнойБазыБазаЗнаний.ПерейтиНаВерсию_1_1_1_0";
	Обработчик.МонопольныйРежим    = Ложь;
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия              = "1.3.1.0";
	Обработчик.Процедура           = "БЗ_ОбновлениеИнформационнойБазыБазаЗнаний.ПерейтиНаВерсию_1_3_1_0";
	Обработчик.МонопольныйРежим    = Ложь;
...
Функция СформироватьАдресСсылкиПоСтроке(СтрокаТаблицы)
	...
	ИначеЕсли СтрокаТаблицы.Тип = 3 Тогда
		АдресСсылки = БЗ_БазаЗнанийAPIКлиентСервер.КонструкторСсылки_page("category", Новый Структура("id", СтрокаТаблицы.Адрес));
	ИначеЕсли СтрокаТаблицы.Тип = 4 Тогда
		АдресСсылки = БЗ_БазаЗнанийAPIКлиентСервер.КонструкторСсылки_search("text", Новый Структура("text", СтрокаТаблицы.Адрес));
...
Процедура ОбработатьГиперссылкиСтатейБазыЗнаний()
	...
	Выборка = Справочники.БЗ_СтатьиБазыЗнаний.Выбрать();
...
Процедура ПеренестиПользователейБазыЗнаний()
	...
	Выборка = Справочники.БЗ_КомментарииБазыЗнаний.Выбрать();
	Пока Выборка.Следующий() Цикл
		ЭлементОбъект = Выборка.ПолучитьОбъект();
		ЭлементОбъект.ОбменДанными.Загрузка = Истина;
	...
	Выборка = Справочники.БЗ_НовостиБазыЗнаний.Выбрать();
	Пока Выборка.Следующий() Цикл
		ЭлементОбъект = Выборка.ПолучитьОбъект();
		ЭлементОбъект.ОбменДанными.Загрузка = Истина;
		ЭлементОбъект.Автор	= ПолучитьПользователя(КэшПользователи, ЭлементОбъект.УдалитьАвтор);
	...
	НаборЗаписей = РегистрыСведений.БЗ_БлокировкаСтатьейБазыЗнаний.СоздатьНаборЗаписей();
	...
	НаборЗаписей = РегистрыСведений.БЗ_ИзбранноеБазыЗнаний.СоздатьНаборЗаписей();
	НаборЗаписей.Прочитать();
	Для Каждого ЗаписьРегистра Из НаборЗаписей Цикл
		ЗаписьРегистра.Пользователь	= ПолучитьПользователя(КэшПользователи, ЗаписьРегистра.УдалитьПользователь);
	КонецЦикла;
	...
	НаборЗаписей = РегистрыСведений.БЗ_ИсторияСтатейБазыЗнаний.СоздатьНаборЗаписей();
	НаборЗаписей.Прочитать();
	Для Каждого ЗаписьРегистра Из НаборЗаписей Цикл
	...
	НаборЗаписей = РегистрыСведений.БЗ_РейтингСтатейБазыЗнаний.СоздатьНаборЗаписей();
	НаборЗаписей.Прочитать();
	Для Каждого ЗаписьРегистра Из НаборЗаписей Цикл
		ЗаписьРегистра.Пользователь	= ПолучитьПользователя(КэшПользователи, ЗаписьРегистра.УдалитьПользователь);
	КонецЦикла;
...
Процедура ОбновитьСтатистикуКатегорийБазыЗнаний()
	...
	БЗ_БазаЗнаний.ОбновитьСтатистикуКатегорий(Новый Структура("Категории", МассивКатегорий));
