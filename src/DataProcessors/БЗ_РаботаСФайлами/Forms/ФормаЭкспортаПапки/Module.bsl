...
Процедура СформироватьДеревоФайлов(ПапкаРодитель)
	...
	|	ВЫРАЗИТЬ(Файлы.ВладелецФайла КАК Справочник.БЗ_ПапкиФайлов).Наименование КАК НаименованиеПапки,
	...
	Иначе
		...
		|			ТОГДА Файлы.Ссылка
		|		ИНАЧЕ ЕстьNull(Файлы.ТекущаяВерсия, ЗНАЧЕНИЕ(Справочник.БЗ_ВерсииФайлов.ПустаяСсылка))
		|	КОНЕЦ";
	...
	Если ПараметрыВыполнения.ФайлНаДиске.Существует() Тогда
		...
		Иначе
			Если Не ПараметрыВыполнения.ОбщиеПараметры.ДляВсехФайлов Тогда
				...
				ОткрытьФорму("Обработка.БЗ_РаботаСФайлами.Форма.ФайлСуществует", СтруктураПараметров, , , , , Обработчик);
				Возврат;
			КонецЕсли;
